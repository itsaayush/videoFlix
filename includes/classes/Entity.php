<?php
    ...
    class Entity{
        ...
        public function getSeasons($seasonNumber){
            $query = $this->conn->prepare("SELECT * from videos WHERE entityId=:id AND isMovie=0 ORDER BY season, episode ASC");
            $query->bindValue(":id", $this->getId());
            if($seasonNumber != null){
                $query = $this->conn->prepare("SELECT * from videos WHERE entityId=:id AND season=:seasonNumber AND isMovie=0 ORDER BY season, episode ASC");
                $query->bindValue(":id", $this->getId());
            ...
            //doing this if for the last time loop is executed
            if(sizeof($videos) != 0){
                $seasons[] = new Season($currentSeason, $videos);
            }
